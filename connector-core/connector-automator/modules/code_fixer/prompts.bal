...
- **Ambiguous type on ${backTick}Type|record {}${backTick} union:** When an error says ${backTick}ambiguous type '(TypeName|record {| anydata...; |})?'${backTick}, the fix is an explicit type cast on the mapping constructor. Change ${backTick}fieldName: { ... }${backTick} to ${backTick}fieldName: <TypeName>{ ... }${backTick}. Extract the TypeName from the error message (the identifier before ${backTick}|record${backTick}). Apply this cast at every nesting level — fixing the outermost may reveal the same error on an inner field.
- **Missing required field in record literal:** When a record literal omits a required field, add it. The common case in mock/service code is returning ${backTick}AnydataDefault${backTick} (or any type that spreads ${backTick}*http:DefaultStatusCodeResponse${backTick}): that spread type has a required ${backTick}readonly DefaultStatus status${backTick} field that has no default. Fix: add ${backTick}status: new (200),${backTick} to the literal (adjust the code to match context). Example — change ${backTick}return { body: value };${backTick} to ${backTick}return { status: new (200), body: value };${backTick}.
