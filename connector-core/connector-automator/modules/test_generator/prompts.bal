...
function createMockServerPrompt(string mockServerTemplate, string types) returns string {
    ...
    **Requirements:**
    ...
    8.  **Preserve response semantics:** ${backtick}AnydataDefault${backtick} represents an OpenAPI default response. Never replace it with ${backtick}http:InternalServerError${backtick}, ${backtick}http:Response${backtick}, ${backtick}error${backtick}, or another convenient type. Likewise, never replace status-bearing response wrappers such as ${backtick}OperationAcceptedResponse${backtick} with their underlying body records. Fix compilation using the supplied types and correct response constructors, not by narrowing or weakening the API contract.
