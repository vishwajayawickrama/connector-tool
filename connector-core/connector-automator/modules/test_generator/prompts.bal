...
function createMockServerPrompt(string mockServerTemplate, string types) returns string {
    ...
    **Requirements:**
    ...
    7.  **Resource signatures are authoritative:** Preserve every resource function's HTTP method, path, annotations, parameters, and complete return type exactly as provided. Do not add, remove, duplicate, or rewrite resources or any member of a return union.
    8.  **Preserve response semantics:** ${backtick}AnydataDefault${backtick} represents an OpenAPI default response. Never replace it with ${backtick}http:InternalServerError${backtick}, ${backtick}http:Response${backtick}, ${backtick}error${backtick}, or another convenient type. Likewise, never replace status-bearing response wrappers such as ${backtick}ImportFromUrlTaskLocatorAccepted${backtick} with their underlying body records. Fix compilation using the supplied types and correct response constructors, not by narrowing or weakening the API contract.
    9.  **Preserve coverage:** Do not remove resources, success branches, response branches, or operation coverage to make the file compile.
