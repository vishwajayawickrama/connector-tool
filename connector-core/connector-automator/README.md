...
- Reviews every operation ID with AI once and preserves the decision across regenerations
...
`ai-mappings.json` is generated by the Ballerina connector tool and must not be edited manually. Its `_warning`
field is always written first. The `operationIds` section stores reviewed operation IDs by exact aligned OpenAPI
path and lowercase HTTP method, while `schemaNames` maps each current aligned schema name to its reviewed name.
Mappings for operations and schemas removed from the current vendor spec are dropped. A changed path or HTTP
method is treated as a new operation. Identity mappings record names that were reviewed and intentionally kept
unchanged:

```json
{
  "_warning": "AUTO-GENERATED FILE. DO NOT MODIFY. This file is generated by the Ballerina connector tool.",
  "operationIds": {
    "/users": {
      "get": "listUsers",
      "post": "createUser"
